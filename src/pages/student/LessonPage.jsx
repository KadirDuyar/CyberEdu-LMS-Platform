...
function renderInline(text) {
  ...
  while (remaining.length > 0) {
    ...
    if (boldIdx <= codeIdx && boldMatch) {
      ...
      parts.push(
        <code key={key++} className="px-1.5 py-0.5 rounded bg-violet-500/20 text-violet-300 text-[0.8em] font-mono">
          {codeMatch[1]}
        </code>
      );
...
export default function LessonPage() {
  ...
  if (completed) {
    if (isCourseFinished) {
      return (
        <DashboardLayout>
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-b from-emerald-950/70 via-slate-900 to-indigo-950/80 border-2 border-emerald-500/40 p-8 md:p-12 text-center max-w-xl mx-auto my-8 shadow-2xl space-y-6">
            ...
            <div className="relative z-10 flex flex-col items-center">
              ...
              <span className="text-xs font-black uppercase tracking-widest text-emerald-400 bg-emerald-950/80 border border-emerald-500/40 px-4 py-1.5 rounded-full inline-block mt-3 mb-2">
                Kurs Tamamlandı!
              </span>

              <h2 className="font-display font-black text-3xl md:text-4xl text-white mt-1">
                Tebrikler, Şampiyon! 🎉
              </h2>
      ...
      <DashboardLayout>
        <div className="flex flex-col items-center justify-center min-h-[70vh] text-center space-y-6 max-w-md mx-auto">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm mt-2">{lesson.title}</p>
          </div>
          ...
          <div className="flex gap-3 w-full">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-white/10 text-slate-300 font-semibold hover:bg-white/20 transition-colors"
            >
              <ArrowLeft size={16}/> Dashboard'a Dön
            </button>
            <button
              onClick={() => navigate('/student/learning-path')}
              className="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-violet-600 hover:bg-violet-500 text-white font-bold transition-all hover:-translate-y-0.5"
            >
        ...
        <div className="flex items-start gap-4 p-5 rounded-2xl glass border border-white/10">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="font-display font-black text-xl text-white">{lesson.title}</h1>
              {alreadyCompleted && (
                ...
                return (
                  <div key={activity.id} className="py-2 animate-in fade-in slide-in-from-bottom-4">
                    {activity.type === 'heading' ? (
                      <h3 className="font-display font-bold text-2xl text-white mt-4 border-b border-white/10 pb-2 mb-4">
                        {activity.question}
                      </h3>
                    ) : (
                  ...
                  className={`glass rounded-2xl border p-5 transition-all duration-500 ${
                    !isUnlocked
                      ...
                      : state?.submitted
                        ...
                        : 'border-violet-500/20 shadow-[0_0_20px_rgba(124,58,237,0.1)]'
                  }`}
                >
